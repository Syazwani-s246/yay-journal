...
const JournalBook: React.FC<JournalBookProps> = ({ entries, isOpen, onClose }) => {
  ...
  //         // Reset direction after page change
  //         setTimeout(() => {
  ...
  // };
    ...
    // Simplify the handlePageTurn function
    const handlePageTurn = (direction: 'next' | 'previous') => {
      if (direction === 'next' && currentPageIndex < groupedEntries.length - 1) {
        setCurrentPageIndex(currentPageIndex + 1);
      } else if (direction === 'previous' && currentPageIndex > 0) {
  ...
  return (
    <div style={{
      ...
      <div style={journalBookStyles.backdrop} onClick={onClose} />
      <div style={journalBookStyles.container}>
      {/* Update the book container style */}
        {/* <div style={{
          ...journalBookStyles.book,
          transform: pageDirection === 'right' ? 'rotateY(-10deg)' :
            pageDirection === 'left' ? 'rotateY(10deg)' : 'none',
        ...
        <div style = {journalBookStyles.book}>
          ...
          <div style={{
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '1rem' }}> {/* Add wrapper div */}
              <h3 style={{ ...journalBookStyles.title, marginBottom: '1rem' }}>
                Weekly Reflection
              </h3>

              <div style={{
                ...journalBookStyles.chart,
                height: '250px',
                width: '100%',
                marginBottom: '1rem'
              }}>
            ...
            {/* Action Buttons */}
            <div style={{
              ...journalBookStyles.actionButtonsContainer,
              position: 'sticky',
              bottom: 0,
              left: 0,
              right: 0,
